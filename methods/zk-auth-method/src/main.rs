...
const ACTION_PREFIX: &[u8] = b"ZK_AUTH_RECORD";
...
fn main() {
    ...
    let identity_commitment = sha256_bytes(&[IDENTITY_PREFIX, &input.secret]);
    let nullifier_hash = sha256_bytes(&[
        NULLIFIER_PREFIX,
        &input.secret,
        &input.nonce,
    ...
    let is_valid = !is_zero_32(&input.secret)
        && !is_zero_32(&input.payload_hash)
        && !is_zero_20(&input.recipient)
        && input.chain_id > 0
        && !is_zero_20(&input.contract_address);
